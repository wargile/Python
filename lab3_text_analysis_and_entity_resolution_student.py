...
def simpleTokenize(string):
    ...
    return [s for s in re.split(split_regex, string.lower()) if len(s) > 0]
...
def tokenize(string):
    ...
    """
    temp = simpleTokenize(string)
    #stopwords_dict = dict.fromkeys(temp, 0)
    stopwords_dict = {}
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (a, b): (a, len(b))).values().sum()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #max_record = vendorRDD.map(lambda (a, b): (a, len(b))).values()
    #tuples = vendorRDD.map(lambda (a, b): (a, len(b))).collect()
    tuples = vendorRDD.map(lambda (a, b): (a, b)).collect()
    #print tuples
    max_val = tuple(('', ''))
    for t in tuples:
        if len(t[1]) > len(max_val[1]):
            max_val = t
    #print max_val
    list_ret = []
    list_ret.append(tuple(max_val))
    #print list_ret
    return list_ret
...
def tf(tokens):
    ...
    for t in tokens_dict:
        tokens_dict[t] = float(tokens_dict[t] / float(len(tokens)))
...
# TODO: Replace <FILL IN> with appropriate code
# TODO: Check your tfidf.py file for code/ideas?
# TIPS:
# The steps your function should perform are:
# * Calculate *N*. Think about how you can calculate *N* from the input RDD.
# * Create an RDD (*not a pair RDD*) containing the unique tokens from each document in the input *corpus*. For each document,
#   you should only include a token once, *even if it appears multiple times in that document.*
# --> ie. for each document you should get a list of unique tokens
# --> combining everything together you should get a list of all tokens across documents
# * For each of the unique tokens, count how many times it appears in the document and then compute the IDF for that token:
#   *N/n(t)*
# --> now you want to count the times the token appears across all documents. how would you do such a count?
...
def idfs(corpus):
    ...
    """
    N = corpus.count() # TODO: Create as broadcast variable, use in func sent to all processes
    N_broadcast = sc.broadcast(N)
    #print N
    #print corpus.take(2)
    uniqueTokens = corpus.map(lambda (a, b): (a, list(set(b)))).map(lambda (a, b): b).flatMap(lambda a: a)
    #print uniqueTokens.collect()[0:20]
    tokenCountPairTuple = uniqueTokens.map(lambda a: (a, 1)).reduceByKey(lambda a, b: a + b) 
    #print tokenCountPairTuple.take(12)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda (a, b): (a, float(N_broadcast.value) / float(b)))
...
def dotprod(a, b):
    ...
    """
    # NOTE: The dot product is defined as the sum of the product of values for tokens that appear in both dictionaries!
    # IMPORTANT! TODO: Make sure your dotprod sums only over keys that exist in both a and b!
    the_dotprod = 0
    
    for key in set(a).intersection(set(b)):
        the_dotprod += a[key] * b[key]
...
def norm(a):
    ...
    Returns:
        norm: the square root of the dot product of a dictionary and itself
    """
    dict_vals = list(a.values())
    #print dict_vals
    vals = 0.0
    for v in dict_vals:
        vals += pow(v, 2)
...
def computeSimilarity(record):
    ...
    #cs = cosineSimilarity(<FILL IN>, idfs_small_weights) # TYPO!
    cs = cosineSimilarity(googleValue, amazonValue, idfsSmallWeights) # NOTE: Changed because of TYPO!
    return (googleURL, amazonID, cs)

similarities = crossSmall.map(lambda a: computeSimilarity(a)).cache()
...
similaritiesBroadcast = crossSmall.map(lambda a: computeSimilarity(a)).cache()
...
sims = similaritiesBroadcast.map(lambda (a, b, c): (str(b + ' ' + a), c))
#print sims.take(2)

# Combine the sims RDD with the goldStandard RDD by creating a new trueDupsRDD RDD that has just the cosine similarity
# scores for those "AmazonID GoogleURL" pairs that appear in both the sims RDD and goldStandard RDD
trueDupsRDD = sims.join(goldStandard) # Hint: you can do this using the join() transformation
trueDupsCount = trueDupsRDD.count()
print trueDupsRDD.take(1)
avgSimDups = trueDupsRDD.map(lambda (a, b): b[0]).reduce(lambda a, b: a + b) / float(trueDupsCount)

# TODO: Filter here to find those NOT in both??
nonDupsRDD = sims.subtractByKey(goldStandard)
nonDupsCount = nonDupsRDD.count()
print "NonDups: " + str(nonDupsCount)
print "NonDups:" + str(nonDupsRDD.take(2))
avgSimNon = nonDupsRDD.map(lambda (a, b): b).sum() / float(nonDupsCount)
#print nonDupsRDD.map(lambda (a, b): b).sum() / float(nonDupsCount)
...
#print idfsFull.take(12)

idfsFullWeights = idfsFull.map(lambda (a, b): (a, b)).collectAsMap() # ??
#print idfsFullWeights[0:2]
idfsFullBroadcast = sc.broadcast(idfsFullWeights)

#print amazonFullRecToToken.take(2)
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (a, b): (a, norm(b))) # Return a dict?
#print amazonWeightsRDD.take(1)
amazonNormsBroadcast = sc.broadcast(amazonNorms.collect())
#print amazonNorms.collect()[0:3]
googleNorms = googleWeightsRDD.map(lambda (a, b): (a, norm(b))) # Return a dict?
...
def invert(record):
    ...
    for token in record[1]:
        token_list.append(tuple((token, record[0])))
        
    return (token_list)

def f(x): return x

amazonInvPairsRDD = amazonWeightsRDD.map(lambda (a, b): invert((a, b))).flatMap(f).cache()
#print amazonInvPairsRDD.take(2)
googleInvPairsRDD = googleWeightsRDD.map(lambda (a, b): invert((a, b))).flatMap(f).cache()
