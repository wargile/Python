...
# https://en.wikipedia.org/?title=Collaborative_filtering
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    return (IDandRatingsTuple[0], (len(IDandRatingsTuple[1]),                                    float(sum(IDandRatingsTuple[1]) / float(len(IDandRatingsTuple[1])))))
...
movieIDsWithRatingsRDD = ratingsRDD.map(lambda (a, b, c): (b, c)).groupByKey()
...
# Output should be: (1.0, u'Autopsy (Macchie Solari) (1975)', 1)
# (2049, (u'Happiest Millionaire, The (1967)', (22, 3.6818181818181817)))
#movieNameWithAvgRatingsRDD = moviesRDD.map(lambda (a, b): (a, b)).join(movieIDsWithAvgRatingsRDD)
movieNameWithAvgRatingsRDD = moviesRDD.map(lambda (a, b): (a, b)).join(movieIDsWithAvgRatingsRDD)     .map(lambda (a, b): (b[1][1], b[0], b[1][0]))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = predictedReformattedRDD.join(actualReformattedRDD)         .map(lambda (a, b): (a, (b[0]-b[1])**2))
    #print squaredErrorsRDD.take(10)
    
    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda (a, b): float(b)).sum()
    ...
    return math.sqrt(float(float(totalError) / float(numRatings)))
...
trainingAvgRating = trainingRDD.map(lambda (a, b, c): c).sum() / float(trainingRDD.count())
print 'The average rating for movies in the training set is %s' % trainingAvgRating

trainingAvgRating_bc = sc.broadcast(trainingAvgRating)

testForAvgRDD = testRDD.map(lambda (a, b, c): (a, b, trainingAvgRating_bc.value))
#print testForAvgRDD.take(2)
testForRMSERDD = testRDD.map(lambda (a, b, c): (a, b, c))
#print testForRMSERDD.take(2)
...
#print predictedTestMyRatingsRDD.take(6)
#print testForPredictingRDD.take(6)
...
myUnratedMoviesRDD = moviesRDD.filter(lambda (a, b): a != myUserID)
...
predictedRDD = predictedRatingsRDD.map(lambda(a, b, c):(a, b))
...
predictedWithCountsRDD  = predictedRDD.join(movieCountsRDD).map(lambda (a, (b, c)): (a, (b, c)))
...
ratingsWithNamesRDD = predictedWithCountsRDD.filter(lambda (a, (b, c)): (a, b, c > 75)) 
