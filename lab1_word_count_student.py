...
# TODO: Replace <FILL IN> with appropriate code
# shakespeareWordsRDD = shakespeareRDD.filter(lambda x: len(x.strip()) > 0).flatMap(lambda x: x.split(' '))
# shakespeareWordsRDD = shakespeareRDD.filter(lambda x: len(x.strip(' ')) > 0).flatMap(lambda x: re.split('\s?', x))
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda x: re.split('\s?', x))
# print shakespeareWordsRDD.take(3)
# shakespeareWordsRDD = shakespeareRDD.flatMap(lambda x: re.split('\s?', x)).filter(lambda x: len(x) > 0)
# shakespeareWordsRDD = shakespeareRDD.filter(lambda x: len(x) > 0).flatMap(lambda x: x.split(' '))
# shakespeareWordsRDD = shakespeareRDD.filter(lambda x: x != '').flatMap(lambda x: x.split(' '))
...
# TIP: See accumulator blank line count example!
