...
def parsePoint(line):
    ...
    """
    elements = line.split(',')
    return LabeledPoint(float(elements[0]), [float(x) for x in elements[1:]])
...
# IMPORTANT! Add seed param for split reproducibility!
...
# TODO: Replace <FILL IN> with appropriate code (NOTE: trainData = LabeledPoint)
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda x: getLabeledPrediction(w, x)) # <FILL IN>
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)
        # Input: LabelsAndPredTrain (RDD of (float, float)): An `RDD` consisting of (label, prediction) tuples

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` should be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda x: gradientSummand(w, x)).sum() # Weights and LabeledPoint
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, alpha, miniBatchFrac, None,                                            reg, regType, useIntercept) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
# testPoint = parsedValData.take(1)[0]
# print testPoint.features
labelsAndPreds = parsedValData.map(lambda p: (p.label, firstModel.predict(p.features))) #<FILL IN>
...
# import itertools
# for i in itertools.product([2,3],repeat=2):
#     print i
# i = itertools.product([2,3],repeat=2)
# print [x[0]*x[1] for x in itertools.product([2,3],repeat=2)]
...
def twoWayInteractions(lp):
    ...
    """
    #lp_interactions = [x[0]*x[1] for x in itertools.product(lp.features, repeat=len(lp.features))]
    lp_interactions = [x[0]*x[1] for x in itertools.product(lp.features, repeat=2)]
    return LabeledPoint(lp.label, np.hstack((lp.features, lp_interactions)))
